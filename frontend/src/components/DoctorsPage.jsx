...
import placeholder from "../assets/placeholder-doctor.jpg";
    ...
    async function load() {
      ...
      try {
        ...
        const normalized = (Array.isArray(items) ? items : []).map((d) => {
          const id = d._id || d.id;
         const rawImage = d.imageUrl || d.image || d.imageSmall || d.imageSrc || "";
        ...
        // if API returned empty, fallback to bundled local data
        const source = normalized && normalized.length > 0 ? normalized : localDoctors;

        // dedupe normalized list by id (fallback to name)
        const uniqueByIdOrName = (arr) => {
          const seen = new Map();
          for (const it of arr) {
        ...
        const unique = uniqueByIdOrName(source);
        ...
        if (mounted) {
          setError("Network error while loading doctors. Using local data.");
