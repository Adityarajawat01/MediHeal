...
//this function will build the frontend base url for redirect after payment
...
//this function will resolve the clerk user id from the request object
...
export const confirmServicePayment = async (req, res) => {
  ...
  export const updateServiceAppointment = async (req, res) => {
    try {
      ...
      //first check wheater fill if yes then update the field
...
export const getServiceAppointmentStats = async (req, res) => {
  try {
    ...
    // cancel appt and total earning done by themselves
    return res.json({
      success: true,
      services,
...
export const getServicesAppointmentsByPatient = async (req, res) => {
  try{
    const clerkUserId = resolveClerkUserId(req);
    ...
    return res.json ({
     ...
     console.error("getServicesAppointmentsByPatient error:", error);
    return res.status(500).json({ success: false, message: "Server error" });
...
export default {
  ...
  getServicesAppointmentsByPatient,
}
