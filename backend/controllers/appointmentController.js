...
//helper function will return finte number
...
//helper function to build frontend base url
...
//this function will get user from clerk and return the user details 
    ...
    if(!clerkUserId) {
      ...
      if(existingBooking) {
        return res.status(409).json({
          success: false,
          message: "an appointment already exists for this doctor at the specified date and time"
        })
      }
      ...
      try{
        ...
        console.warn("Doctor not found:", e?.message || e);
      }
      if(!doctor) {
...
//to update the appointment details by doctor or patient
...
//updateAppointment 
    ...
    ).populate({path: "doctorId", select:"name speciality owner imageUrl" }).lean();
